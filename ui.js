...
const UI = {
  ...
  setErrorHero(lang) {
    ...
    document.getElementById('weatherIcon').className = 'bi hero__icon wx-icon bi-wifi-off';
    document.getElementById('currentTemp').textContent = '--';
    document.getElementById('weatherDesc').textContent = I18N[lang].loadError;
    document.getElementById('feelsLike').textContent = '';
    document.getElementById('stickyCity').textContent = I18N[lang].noData;
    document.getElementById('stickyTemp').textContent = '--°';
    document.getElementById('stickyIcon').className = 'bi wx-icon sticky-weather__icon bi-wifi-off';
  },
  ...
  renderHero(city, current, todayDaily, lang, unit) {
    ...
    document.getElementById('weatherIcon').className = `bi hero__icon wx-icon ${icon}`;
    document.getElementById('currentTemp').textContent = WeatherUtils.formatTemp(current.temperature_2m, unit);
    document.getElementById('weatherDesc').textContent = WeatherUtils.describe(current.weather_code, lang, current.visibility);
    document.getElementById('feelsLike').textContent = `${WeatherUtils.formatTemp(current.apparent_temperature, unit)}°`;

    // Mantém o cabeçalho fixo (visível durante o scroll) sincronizado
    // com os mesmos dados do hero.
    document.getElementById('stickyCity').textContent = city.name;
    document.getElementById('stickyIcon').className = `bi wx-icon sticky-weather__icon ${icon}`;
    document.getElementById('stickyTemp').textContent = `${WeatherUtils.formatTemp(current.temperature_2m, unit)}°`;
  ...
  renderHourly(hourly, timezone, lang, unit, nowIso) {
    ...
    slice.forEach((iso, i) => {
      ...
      el.innerHTML = `
        ...
        <i class="bi wx-icon ${icon} hour-card__icon"></i>
  ...
  renderDaily(daily, lang, unit) {
    ...
    daily.time.forEach((iso, i) => {
      ...
      el.innerHTML = `
        ...
        <i class="bi wx-icon ${icon} day-row__icon"></i>
  ...
  renderSevereAlert(alert, lang, nowIso) {
    ...
    document.getElementById('severeAlertDetail').textContent = alert.gusts
      ...
      : WeatherUtils.formatAlertWindow(alert.startIso, nowIso, lang);
  ...
  // -------------------------------------------------------------------
  // Dica diária (agora acionável: já vem com ícone + texto formados)
  // -------------------------------------------------------------------
  renderDailyTip(tip, show) {
    const el = document.getElementById('dailyTip');
    if (!show || !tip) { el.hidden = true; return; }
    el.innerHTML = `<i class="bi ${tip.icon || 'bi-lightbulb'}"></i> ${tip.text}`;
    el.hidden = false;
  },
  ...
  // -------------------------------------------------------------------
  renderRainAlert(rainInfo) {
    const banner = document.getElementById('rainAlert');
    if (!banner) return;
    // Só chama a atenção quando a chuva é iminente (agora ou nas próximas
    // ~2h) — quando não há chuva prevista, não vale a pena ocupar espaço.
    if (!rainInfo || rainInfo.status === 'none') { banner.hidden = true; return; }
    banner.hidden = false;
    document.getElementById('rainAlertIcon').className = `bi wx-icon ${rainInfo.icon}`;
    document.getElementById('rainAlertTitle').textContent = rainInfo.text;
  },

  // -------------------------------------------------------------------
  // Comparação histórica ("hoje está X° acima/abaixo da média")
  // -------------------------------------------------------------------
  renderHistoricalCompare(compare, city, lang, unit) {
    const el = document.getElementById('historicalCompare');
    if (!el) return;
    if (!compare) { el.hidden = true; return; }
    const diff = WeatherUtils.formatTempDiff(compare.diff, unit);
    const cityName = city ? city.name : '';
    let text;
    if (Math.abs(diff) < 1) {
      text = T(lang, 'historicalNormal', { city: cityName });
    } else if (diff > 0) {
      text = T(lang, 'historicalAbove', { diff: `+${diff}`, city: cityName });
    } else {
      text = T(lang, 'historicalBelow', { diff: Math.abs(diff), city: cityName });
    }
    const icon = diff > 0.5 ? 'bi-graph-up-arrow' : (diff < -0.5 ? 'bi-graph-down-arrow' : 'bi-dash-circle');
    el.innerHTML = `<i class="bi ${icon}"></i> ${text}`;
    el.dataset.tone = diff > 0.5 ? 'above' : (diff < -0.5 ? 'below' : 'normal');
    el.hidden = false;
  },
  ...
  applyTranslations(lang) {
    ...
    set('labelLocateNow', d.locateSettingsLabel);
    ...
    set('shareSheetTitle', d.shareSheetTitle);
    set('shareOptWhatsapp', d.shareWhatsapp);
    set('shareOptFacebook', d.shareFacebook);
    set('shareOptTwitter', d.shareTwitter);
    set('shareOptTelegram', d.shareTelegram);
    set('shareOptEmail', d.shareEmail);
    set('shareOptCopy', d.shareCopyLink);
    set('shareOptDownload', d.shareDownloadImg);
    set('shareOptMore', d.shareMoreApps);
