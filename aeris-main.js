...
const App = {
  ...
  async loadCity(city, opts = {}) {
    ...
    const [weather, aqi] = await Promise.all([
      Api.getWeather(city.lat, city.lon, city.timezone, !!opts.forceFresh),
      Api.getAirQuality(city.lat, city.lon, !!opts.forceFresh),
    ]);
  ...
  applySettingsToUI() {
    ...
    document.querySelectorAll('#segLang button').forEach(b => b.classList.toggle('active', b.dataset.val === s.lang));
    ...
    document.getElementById('searchInput').placeholder = I18N[s.lang].searchPlaceholder;
  },
  ...
  bindEvents() {
    ...
    document.getElementById('segLang').addEventListener('click', (e) => {
      const btn = e.target.closest('button'); if (!btn) return;
      this.saveSettings({ lang: btn.dataset.val });
    });
...
// ===========================================================================
// Particles — leve efeito ambiente (neve / chuva / brilho) no canvas de fundo
// ===========================================================================
const Particles = {
  canvas: null, ctx: null, items: [], theme: 'clear', raf: null,
  ...
  seed() {
    ...
    const count = /snow/.test(this.theme) ? 46 : /rain/.test(this.theme) ? 60 : /storm/.test(this.theme) ? 70 : 18;
  ...
  loop() {
    ...
    const isSnow = /snow/.test(this.theme);
    const isRain = /rain/.test(this.theme) || /storm/.test(this.theme);
...
// Como os scripts agora podem ser carregados de forma assíncrona (ver
// carregador sequencial no index.html), o evento "DOMContentLoaded" pode já
// ter disparado antes deste ficheiro carregar. Por isso verificamos o
// estado atual do documento em vez de confiar apenas no evento.
